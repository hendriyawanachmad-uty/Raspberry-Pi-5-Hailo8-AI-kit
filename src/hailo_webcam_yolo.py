...
# Threshold YOLO
CONF_THRESH = 0.6           # cukup rendah untuk memastikan deteksi muncul
NMS_IOU_THRESH = 0.5
...
def init_camera(index):
    ...
    if not cap.isOpened():
        raise RuntimeError(f"Tidak bisa membuka kamera di index {index} (/dev/video{index})")
    ...
    print(f"[INFO] Kamera siap di /dev/video{index}")
    print(f"[INFO] Resolusi kamera: {width}x{height}, FPS ~ {fps:.1f}")
    return cap
...
def postprocess_detections_yolo_nms(output_raw, frame_shape):
    """
    Decoder khusus untuk output YOLOv8 NMS Hailo (sesuai pola Python):
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python hailo_webcam_yolo.py <model.hef> [cam_index]")
        print("Contoh: python hailo_webcam_yolo.py yolov8s.hef 8")
        sys.exit(1)
    ...
    if len(sys.argv) >= 3:
        ...
        cam_index = 0  # default /dev/video0

    print(f"[INFO] Memuat HEF: {hef_path}")
    print(f"[INFO] Menggunakan kamera index: {cam_index} (/dev/video{cam_index})")
    ...
    with hpf.VDevice() as vdev:
        ...
        # Input: UINT8 (quantized=False, format_type=UINT8)
        ...
        with network_group.activate(network_group_params):
            with hpf.InferVStreams(
                ...
                print("[INFO] Mulai streaming dari kamera. Tekan 'q' untuk keluar.")
                prev_time = time.time()
                ...
                while True:
                    ...
                    cv2.imshow("Hailo YOLO Inference - Webcam", vis_frame)
